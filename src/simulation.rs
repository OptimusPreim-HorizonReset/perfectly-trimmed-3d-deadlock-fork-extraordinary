...
impl Simulation {
    ...
    pub(crate) fn record_gpdm_event(&mut self, event: crate::gpdm::host::HostEvent) {
        self.gpdm_event_buffer.push(event);
    }

    pub(crate) fn record_body_state_change<I>(&mut self, operation: &'static str, ids: I)
    where
        I: IntoIterator<Item = u64>,
    {
        ...
        self.record_gpdm_event(crate::gpdm::host::HostEvent::StateChanged { ids, operation });
    }

    pub(crate) fn clear_bodies_with_gpdm(&mut self) {
        let removed_ids: Vec<u64> = self.bodies.iter().map(|body| body.id).collect();
        if removed_ids.is_empty() {
            self.bodies.clear();
            return;
        }

        self.record_body_state_change("clear_bodies", removed_ids.iter().copied());
        for id in &removed_ids {
            self.record_gpdm_event(crate::gpdm::host::HostEvent::Removed { id: *id });
        }
        self.bodies.clear();
    }

    pub(crate) fn replace_bodies_with_gpdm(&mut self, bodies: Vec<Body>) {
        let removed_ids: Vec<u64> = self.bodies.iter().map(|body| body.id).collect();
        let spawned_ids: Vec<u64> = bodies.iter().map(|body| body.id).collect();

        if !removed_ids.is_empty() {
            self.record_body_state_change("replace_bodies_remove", removed_ids.iter().copied());
            for id in &removed_ids {
                self.record_gpdm_event(crate::gpdm::host::HostEvent::Removed { id: *id });
            }
        }
        ...
        if !spawned_ids.is_empty() {
            self.record_body_state_change("replace_bodies_spawn", spawned_ids.iter().copied());
            for id in &spawned_ids {
                self.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: *id });
            }
        }
    ...
    fn apply_gpdm_effects(&mut self, effects: crate::gpdm::host::HostEffects) {
        ...
        for imp in effects.velocity_impulses {
            if let Some(body_idx) = self.bodies.iter().position(|b| b.id == imp.participant_id) {
                self.record_body_state_change("apply_velocity_impulse", [imp.participant_id]);
                // sanitize impulse components
                let impulse = Self::sanitize_vec(imp.impulse);
                self.bodies[body_idx].vel += impulse;
            }
        }

        // Gas heating
        for heat in effects.gas_heat {
            if let Some(body_idx) = self.bodies.iter().position(|b| b.id == heat.participant_id) {
                if self.bodies[body_idx].segment_type == crate::body::ParticleSegmentType::Gas {
                    self.record_body_state_change("apply_gas_heat", [heat.participant_id]);
                    let added = (heat.heat as f32).max(0.0);
                    self.bodies[body_idx].gas_temperature =
                        (self.bodies[body_idx].gas_temperature + added).max(0.0);
                }
            }
        }
        ...
        for mt in effects.mass_transfers {
            ...
            if let Some(fi) = from_idx {
                if self.bodies[fi].mass >= amount - 1e-9 {
                    self.record_body_state_change("apply_mass_transfer", [mt.from, mt.to]);
                ...
                self.record_body_state_change("credit_mass_transfer_target", [mt.to]);
                self.bodies[ti].mass += amount;
            }
        }
    }
            ...
            for ev in &events {
                match ev {
                    crate::gpdm::host::HostEvent::Spawned { id } => {
                        ...
                        // If Phase-0 bootstrapping is enabled, seed the account from the host body mass when available.
                        let initial_balance = if self.config.gpdm_bootstrap_accounts_from_body_mass {
                            self.bodies.iter().find(|b| b.id == *id).map(|b| b.mass as f64).unwrap_or(0.0)
                        } else { 0.0 };
                        crate::gpdm::accounts::ensure_account(&mut self.gpdm_accounts, *id, initial_balance);
                        ...
                        if !self.gpdm_participants.contains_key(id) {
                            ...
                            // If a participant already exists (e.g., id reuse after removal), only
                            // reactivate it if the configured tombstone grace period has elapsed.
                            if let Some(p) = self.gpdm_participants.get_mut(id) {
                                let can_activate = p.can_reactivate(self.frame, self.config.gpdm_participant_tombstone_grace_frames);
                                if can_activate {
                                    if let Some(body) = self.bodies.iter().find(|b| b.id == *id) {
                                        p.active = true;
                                        p.position = body.pos;
                                    ...
                                    eprintln!("[GPDM] Ignoring Spawned for id={} during tombstone grace (removed at {:?}, frame={})", id, p.removed_frame, self.frame);
                                }
                            }
                        }
                    }
                    crate::gpdm::host::HostEvent::Merged { survivor, removed } => {
                        // Ensure accounts present for both sides; seed from host body mass if configured.
                        let survivor_initial = if self.config.gpdm_bootstrap_accounts_from_body_mass {
                            self.bodies.iter().find(|b| b.id == *survivor).map(|b| b.mass as f64).unwrap_or(0.0)
                        } else { 0.0 };
                        let removed_initial = if self.config.gpdm_bootstrap_accounts_from_body_mass {
                            self.bodies.iter().find(|b| b.id == *removed).map(|b| b.mass as f64).unwrap_or(0.0)
                        } else { 0.0 };
                        crate::gpdm::accounts::ensure_account(&mut self.gpdm_accounts, *survivor, survivor_initial);
                        crate::gpdm::accounts::ensure_account(&mut self.gpdm_accounts, *removed, removed_initial);
    ...
    fn process_gas_ignition(&mut self) {
        ...
        for idx in ignition_indices {
            ...
            if remaining_mass <= self.config.gas_ignition_mass {
                self.record_body_state_change("gas_ignition_replace", [gas.id, star.id]);
                self.record_gpdm_event(crate::gpdm::host::HostEvent::Removed { id: gas.id });
                ...
                self.record_body_state_change(
                    "gas_ignition_split_remaining_gas",
                    [remaining_gas.id, star.id],
                );
    ...
    fn merge_particle_bodies(&mut self, i: usize, j: usize) {
        ...
        self.record_body_state_change("merge_particle_replace", [b1.id, merged_body.id]);
        ...
        self.record_body_state_change("merge_particle_finalize", [b1.id, merged_body.id]);
    ...
    fn shift_molecule_indices_after_removal(&mut self, removed_idx: usize) {
        ...
        let cleared_molecule_ids: Vec<u64> = self
            .bodies
            .iter()
            .filter_map(|body| {
                body.molecule_id.and_then(|molecule_id| {
                    (!self.molecules.iter().any(|m| m.id == molecule_id)).then_some(body.id)
                })
            })
            .collect();
        if !cleared_molecule_ids.is_empty() {
            self.record_body_state_change(
                "clear_orphaned_molecule_membership",
                cleared_molecule_ids,
            );
        }
    ...
    fn merge_center_particles(&mut self, i: usize, j: usize) {
        ...
        self.record_body_state_change("merge_center_replace", [b1.id, merged_body.id]);
        self.bodies[first] = merged_body;
        self.bodies.remove(second);
        self.shift_contact_frames_after_removal(second);
        self.shift_molecule_indices_after_removal(second);
...
mod tests {
    ...
    fn core_particles_merge_only_after_more_than_five_frames_of_contact() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: body1.id });
        sim.bodies.push(body1);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: body2.id });
    ...
    fn collide_handles_reused_collision_grid_buffers() {
        ...
        for i in 0..8 {
            ...
            sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: body.id });
    ...
    fn accretion_spawn_uses_galaxy_rotation_axis_plane() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: center.id });
        sim.bodies.push(center);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: orbital.id });
    ...
    fn collide_reuses_collision_grid_without_panic() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: b_left.id });
        sim.bodies.push(b_left);
        let b_right = Body::new(
            Vec3::new(10.0, 0.0, 0.0),
            Vec3::zero(),
            1.0,
            1.0,
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: b_right.id });
        sim.bodies.push(b_right);
    ...
    fn gas_update_computes_density_and_cooling() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: g1.id });
        sim.bodies.push(g1);
        let g2 = Body::new(
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: g2.id });
        sim.bodies.push(g2);
    ...
    fn gas_rotation_relaxation_damps_out_of_plane_velocity() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: gas.id });
        sim.bodies.push(gas);
        let gas2 = Body::new(
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: gas2.id });
        sim.bodies.push(gas2);
    ...
    fn molecular_bonding_forms_sodium_chloride() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: na.id });
        sim.bodies.push(na);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: cl.id });
        sim.bodies.push(cl);
    ...
    fn molecular_bonding_does_not_form_for_noble_gas() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: he.id });
        sim.bodies.push(he);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: ne.id });
        sim.bodies.push(ne);
    ...
    fn molecule_preserves_total_mass() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: h1.id });
        sim.bodies.push(h1);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: h2.id });
        sim.bodies.push(h2);
    ...
    fn non_diatomic_same_element_still_merges_with_molecular_bonding() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: fe1.id });
        sim.bodies.push(fe1);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: fe2.id });
        sim.bodies.push(fe2);
    ...
    fn gpdm_event_tick_apply_roundtrip() {
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: b1.id });
        sim.bodies.push(b1);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: b2.id });
        sim.bodies.push(b2);
    ...
    fn gpdm_spawn_reactivates_removed_participant() {
        ...
        config.enable_gpdm = true;
        config.gpdm_tick_interval = 1;
        // For this test, allow immediate reactivation (no tombstone grace).
        config.gpdm_participant_tombstone_grace_frames = 0;
    ...
    #[test]
    fn gpdm_merge_records_merged_event() {
        let mut sim = Simulation::new(&InformationsConfig::default());
        sim.clear_bodies_with_gpdm();
        sim.gpdm_event_buffer.clear();
        ...
        // Prepare octree so merge paths that sample space (spawn/accretion) do not
        // panic when they query the octree. Some merge variants consult spatial
        // structures; initialising a minimal octree avoids index-out-of-bounds in tests.
        let oct = Oct::new_containing(&sim.bodies);
        sim.octree.clear(oct);
        for body in &sim.bodies {
            sim.octree.insert(body.pos, body.mass);
        ...
        // Call private merge helper directly to exercise the merge path and its
        // instrumentation (records a Merged event before mutating the bodies Vec).
        sim.merge_particle_bodies(0, 1);

        let has_merged = sim
            .gpdm_event_buffer
            .iter()
            .any(|ev| matches!(ev, crate::gpdm::host::HostEvent::Merged { survivor, removed } if *survivor == id1 && *removed == id2));
        assert!(has_merged, "merge_particle_bodies should record a Merged HostEvent");
    }

    #[test]
    fn gpdm_process_gas_ignition_records_gas_ignited_event() {
        let mut config = InformationsConfig::default();
        config.gas_enabled = true;
        config.gas_volume_enabled = true;
        // Lower thresholds to make the test deterministic / cheap
        config.gas_ignition_mass = 0.5;
        config.gas_ignition_temperature = 0.5;
        config.gas_ignition_density = 0.0;
        ...
        // Directly exercise the ignition pass and assert the HostEvent was recorded
        sim.process_gas_ignition();

        let has_ignited = sim
            .gpdm_event_buffer
            .iter()
            .any(|ev| matches!(ev, crate::gpdm::host::HostEvent::GasIgnited { .. }));
        assert!(has_ignited, "process_gas_ignition should record a GasIgnited HostEvent");
    }

    #[test]
    fn gpdm_clear_bodies_records_state_change_and_removals() {
        let mut sim = Simulation::new(&InformationsConfig::default());
        sim.clear_bodies_with_gpdm();
        sim.gpdm_event_buffer.clear();
        ...
        assert!(sim.bodies.is_empty());
        assert!(matches!(
            sim.gpdm_event_buffer.first(),
            Some(crate::gpdm::host::HostEvent::StateChanged { operation, .. }) if *operation == "clear_bodies"
        ));
        assert_eq!(
            sim.gpdm_event_buffer
                .iter()
                .filter(|ev| matches!(ev, crate::gpdm::host::HostEvent::Removed { .. }))
                .count(),
            2
        );
    }
    ...
    #[test]
    fn gpdm_replace_bodies_records_removed_then_spawned_events() {
        let mut sim = Simulation::new(&InformationsConfig::default());
        sim.clear_bodies_with_gpdm();
        sim.gpdm_event_buffer.clear();

        let existing = Body::new(
            Vec3::zero(),
            Vec3::zero(),
            1.0,
            1.0,
        ...
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: existing.id });
        sim.bodies.push(existing);
        sim.gpdm_event_buffer.clear();

        let replacement = Body::new(
        ...
        sim.replace_bodies_with_gpdm(vec![replacement]);

        assert_eq!(sim.bodies.len(), 1);
        assert_eq!(sim.bodies[0].id, replacement_id);
        assert!(matches!(
            sim.gpdm_event_buffer.first(),
            Some(crate::gpdm::host::HostEvent::StateChanged { operation, .. })
                if *operation == "replace_bodies_remove"
        ));
        assert!(sim
            .gpdm_event_buffer
            .iter()
            .any(|ev| matches!(ev, crate::gpdm::host::HostEvent::Removed { .. })));
        assert!(matches!(
            sim.gpdm_event_buffer.last(),
            Some(crate::gpdm::host::HostEvent::Spawned { id }) if *id == replacement_id
        ));
    }

    #[test]
    fn gpdm_apply_effects_records_velocity_state_change() {
        let mut config = InformationsConfig::default();
        config.enable_gpdm = true;
        let mut sim = Simulation::new(&config);
        sim.clear_bodies_with_gpdm();
        ...
        let id = body.id;
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id });
        sim.bodies.push(body);
        sim.gpdm_event_buffer.clear();

        sim.apply_gpdm_effects(crate::gpdm::host::HostEffects {
            velocity_impulses: vec![crate::gpdm::host::VelocityImpulse {
                participant_id: id,
        ...
        assert!(sim.gpdm_event_buffer.iter().any(|ev| {
            matches!(
                ev,
                crate::gpdm::host::HostEvent::StateChanged { ids, operation }
                    if *operation == "apply_velocity_impulse" && ids == &vec![id]
            )
        }));
    }

    #[test]
    fn gpdm_apply_effects_records_gas_heat_state_change() {
        let mut config = InformationsConfig::default();
        config.enable_gpdm = true;
        let mut sim = Simulation::new(&config);
        sim.clear_bodies_with_gpdm();
        ...
        sim.apply_gpdm_effects(crate::gpdm::host::HostEffects {
            gas_heat: vec![crate::gpdm::host::GasHeat {
                participant_id: id,
                heat: 2.0,
            }],
            ..Default::default()
        ...
        assert!(sim.bodies[0].gas_temperature > 0.25);
        assert!(sim.gpdm_event_buffer.iter().any(|ev| {
            matches!(
                ev,
                crate::gpdm::host::HostEvent::StateChanged { ids, operation }
                    if *operation == "apply_gas_heat" && ids == &vec![id]
            )
        }));
    }

    #[test]
    fn gpdm_apply_effects_records_mass_transfer_state_change() {
        let mut config = InformationsConfig::default();
        config.enable_gpdm = true;
        let mut sim = Simulation::new(&config);
        sim.clear_bodies_with_gpdm();
        ...
        let from_id = from.id;
        let to_id = to.id;
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: from_id });
        sim.bodies.push(from);
        sim.record_gpdm_event(crate::gpdm::host::HostEvent::Spawned { id: to_id });
        sim.bodies.push(to);
        sim.gpdm_event_buffer.clear();

        sim.apply_gpdm_effects(crate::gpdm::host::HostEffects {
            mass_transfers: vec![crate::gpdm::host::MassTransfer {
                from: from_id,
                to: to_id,
                mass: 1.5,
            }],
        ...
        assert!(sim.bodies[0].mass < 4.0);
        assert!(sim.bodies[1].mass > 1.0);
        assert!(sim.gpdm_event_buffer.iter().any(|ev| {
            matches!(
                ev,
                crate::gpdm::host::HostEvent::StateChanged { ids, operation }
                    if *operation == "apply_mass_transfer" && ids == &vec![from_id, to_id]
            )
        }));
    }
}
